...
/*   Updated: 2024/09/15 20:15:11 by kyeh             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	initialize_sem(t_info *info)
{
	info->show = sem_open("/sem_show", O_CREAT, 0644, 1);
	info->check = sem_open("/sem_check", O_CREAT, 0644, 1);
	info->fork = sem_open("/sem_fork", O_CREAT, 0644, info->nb_philo);
	if (info->show == SEM_FAILED || info->check == SEM_FAILED
		|| info->fork == SEM_FAILED)
	{
