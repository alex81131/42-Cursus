...
BitcoinExchange::BitcoinExchange(const BitcoinExchange& src): _mapExchangeRate(src._mapExchangeRate)
{
...
BitcoinExchange&	BitcoinExchange::operator = (const BitcoinExchange& src)
{
	...
	if (this != &src)
		_mapExchangeRate = src._mapExchangeRate;
	return *this;
}
...
{
	return _mapExchangeRate;
}

void	BitcoinExchange::readDataCSV(void)
{
	std::ifstream	file(database_path.c_str())	// string to char*
	if (!isAccessible(file, database_path))
		return ;
	...
	while (std::getline(file, line))
	{
		...
		std::istringstreasm(rateStr) >> rate;	// string to double
		_mapExchangeRate[date] = rate;			// set map[date] = rate
	}
	file.close();
}
...
bool	BitcoinExchange::isAccessible(std::ifstream& input, const std::string& fileName) const
{
	...
	const std::ifstrean::po_type	maxSize = 10000000		// 10 MG
