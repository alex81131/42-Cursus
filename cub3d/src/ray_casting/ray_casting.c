...
/*   Updated: 2024/10/09 15:55:51 by kyeh             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	rc_ray_distance(t_vec *ray, t_data *data)
{
	...
	if (data->side == 1)
		return ((ray_d.y - unit_d.y) * ray_cos(ray, &data->ava_dir));
	else
		return ((ray_d.x - unit_d.x) * ray_cos(ray, &data->ava_dir));
}
...
int	rc_rendering(t_data *data)
{
	...
	while (++x < WIDTH)
	{
		...
		data->ray_dir.x = data->p_dir.x + data->ava_cam.x * cam_x;
		data->ray_dir.y = data->p_dir.y + data->ava_cam.y * cam_x;
